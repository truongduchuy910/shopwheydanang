...
import { ROOT, invalidateFields } from '@jesstelford/apollo-cache-invalidation';
...
const queriesToPath = queries => queries.map(query => [ROOT, new RegExp("^".concat(query))]); // Turns this
...
const KeystoneQuery = (_ref2) => {
  ...
  return React.createElement(Query, props, renderProps => React.createElement(KeystoneApolloQueryCacheBuster, _extends({}, renderProps, {
    children: children
  })));
};
...
class KeystoneMutation extends React.Component {
  render() {
    ...
    return React.createElement(SchemaContext.Consumer, null, keystoneSchemaInfo => {
      ...
      return React.createElement(Mutation, _extends({}, props, {
...
const flattenApollo = options => adopt(mapValues(options, adoption => {
...
export { KeystoneProvider, KeystoneMutation as Mutation, KeystoneQuery as Query, flattenApollo, injectIsOptimisticFlag, withKeystone };
